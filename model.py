...
class PositionalEmbedding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float):
        ...
        # creating a vector shape (seq_len, 1)
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        x, self_attention_scores = MultiheadAttention.attention(query, key, value, mask, self.dropout)
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, self_attention_block: MultiheadAttention, cross_attention_block: MultiheadAttention,
        ...
        super().__init__()
        self.self_attention_block  = self_attention_block
        self.cross_attettion_block = cross_attention_block
        self.feed_forward_block = feed_forward_block
        self.residual_connections = nn.ModuleList([ResidualConnection(dropout) for _ in range(3)])

    def forward(self, x, encoder_output, src_mask, tgt_mask):
        x = self.residual_connections[0](x, lambda x: self.self_attention_block(x, x, x, tgt_mask))
        x = self.residual_connections[1](x, lambda x: self.cross_attettion_block(x, encoder_output, encoder_output, src_mask))
...
class Transformers(nn.Module):
    ...
    def __init__(self, encoder: Encoder, decoder: Decoder, src_embedding: InputEmbeddings, tgt_embedding: InputEmbeddings, 
        ...
        self.projecttion_layer = projection_layer
    ...
    def project(self, x):
        return self.projecttion_layer(x)
    


def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, 
                      d_model: int = 512, N: int = 6, h: int = 8, dropout: float=0.1,  d_ff: int = 2048) -> Transformers:
    ...
    # Create the prohjection layer
    projection_layer = ProjectionLayer(d_model, tgt_vocab_size)

    # Creating the transformers
    transformers = Transformers(encoder, decoder, src_embed, tgt_embed, src_pos, tgt_pos, projection_layer)
